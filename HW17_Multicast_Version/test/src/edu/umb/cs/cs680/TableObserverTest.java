...
public class TableObserverTest {
	 ...
	 * 
	 *
	 * For the testing TableObserver updateStock method purpose I took two different Ticker Value and Quote value.	
	 * And with the given value
	*/
	@Test
	public void testTableObserver_UpdateStock() {
		
		StockEventObservable seo = new StockEventObservable();
		TableObserver to = new TableObserver();
		seo.addEventListner(to);
		seo.notifyObservers(new StockEvent("ABC",100.00f));
		seo.notifyObservers(new StockEvent("ABC", 111.00f));
		String expected = "TableObserver Stock Name:ABC\nTableObserver Stock Quote:100.0TableObserver Stock Name:ABC\nTableObserver Stock Quote:111.0";
